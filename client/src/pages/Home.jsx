...
const Home = () => {
	...
	const fetchPosts = async () => {
		...
		try {
			const response = await fetch(
				"https://piexelwise.onrender.com/api/v1/post",
				{
					method: "GET",
					headers: {
			...
			if (response.ok) {
				const result = await response.json(); //fetching the data on screen
				setAllPosts(result.data.reverse()); //reason for reversing is that we are showing the newst post at top
			}
		} catch (err) {
			alert(err);
		} finally {
	...
	const handleSearchChange = (e) => {
		//helps in searching the image on homepage
		...
		<section className="max-w-7xl mx-auto">
			<div>
				...
				<p className="mt-2 text-[#666e75] text-[14px] max-w-[500px]">
					...
					images generated by PixelWise AI
				</p>
			</div>
